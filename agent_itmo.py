...
2. is_finish: КРИТИЧЕСКИ ВАЖНО! Определи, хочет ли кандидат завершить интервью
   - Ищи фразы: "стоп", "закончить", "завершить", "конец", "хватит", "достаточно", "stop"
   - Даже если есть пробелы типа " стоп интервью " - это ЗАВЕРШЕНИЕ
   - Если нашел ЛЮБУЮ из этих фраз - ОБЯЗАТЕЛЬНО поставь СТРОГО 'yes'
   - Если НЕ нашел - поставь 'no'

3. difficulty_adjustment: Как изменить сложность следующего вопроса?
   - 'easier' - если кандидат неуверен, ошибается, не знает базовых вещей
...
4. detected_off_topic: Пытался ли кандидат увести разговор от технических вопросов?
   - true - если говорил о нерелевантном, пытался сменить тему
   - false - если отвечал по делу

5. confidence_level: Уровень уверенности в ответе
   - 'uncertain' - не уверен, много «не знаю», «может быть»
   - 'moderate' - средняя уверенность
   - 'confident' - уверенный, четкий ответ

Ответ только на русском языке

Будь объективным и честным в оценке. ВАЖНО! Если кандидат задает встречный вопрос, но он касается работы в компании - ты обязан ответить. Все вопросы не по теме трудоустройства игнорируй, веди диалог по собеседованию.
ВАЖНО: Если в ответе есть слова "стоп", "закончить", "завершить" - is_finish ДОЛЖЕН быть 'yes'!
'''
